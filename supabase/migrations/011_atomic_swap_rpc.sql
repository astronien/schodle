-- Atomic swap RPC
-- ---------------
-- Swaps shift_type_id between two schedule entries on the same date in a
-- single transaction. Prevents partial swaps when the network drops or
-- the second update fails after the first succeeds.
--
-- Returns the updated schedules. Raises an exception on any validation
-- failure so the transaction rolls back automatically.
...
-- Only managers/admins may invoke the swap. Anyone who can call it
-- has already been authenticated via the Edge Function session token.
...
comment on function public.swap_schedule_shifts(uuid, uuid) is
  'Atomically swap shift_type_id between two schedule entries on the same date.';
