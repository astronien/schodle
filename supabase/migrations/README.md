...
รันตามลำดับเลขไฟล์: `001 → 002 → … → 027`
...
- **`026`** เพิ่มคอลัมน์ `shift_types.group_targets` (jsonb) — จำนวนคนเป้าหมายแยกตามกลุ่มตำแหน่ง
  เพื่อให้ AI จัดตารางแยกรายกลุ่ม ถ้าไม่ตั้งค่าจะ fallback ไปใช้ `target_staff` รวมเหมือนเดิม
- **`027`** เพิ่มคอลัมน์ `shift_types.boundary_role` (`'opening'` / `'closing'` / `NULL`) — เลือกเองได้ว่ากะไหนคือกะเปิดร้าน
  กะไหนคือกะปิดร้าน (ตั้งค่าได้ที่ ตั้งค่า → ประเภทกะ) แทนที่จะให้ AI เดาจากเวลาเข้า–เลิกงาน
  ไม่ backfill — ค่าเดิมเป็น `NULL` ทั้งหมด แล้ว AI จะ fallback ไปเดาจากเวลาเหมือนเดิม
...
1. ตั้งชื่อไฟล์เลขถัดไป (ปัจจุบันคือ `027_…`) พร้อมคำอธิบายสั้น ๆ
